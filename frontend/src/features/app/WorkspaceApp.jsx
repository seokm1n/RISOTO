  ...
  return <main className={`min-h-screen ${isAdmin ? "admin-app" : "general-app"}`}>
    <header className="topbar">
      ...
      <div className="topbar-actions">
        ...
        <button className="notification-siren" type="button" onClick={() => { loadNotifications(); setNotificationOpen(true); }} aria-label={`읽지 않은 위험 알림 ${notificationUnreadTotal}건`} aria-expanded={notificationOpen} aria-controls="notification-drawer" title="위험 알림 보기"><Icon name="siren" tone="inherit" />{notificationUnreadTotal > 0 && <span className="notification-badge" aria-hidden="true">{notificationUnreadTotal > 99 ? "99+" : notificationUnreadTotal}</span>}</button>
