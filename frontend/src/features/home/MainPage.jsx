...
export default function MainPage({ onOpenCompany, onEditCompany }) {
  ...
  return <section className="workspace main-workspace">
    <div className="workspace-head">
      <div><span className="eyebrow">MY COMPANY</span><h1>{mainCompany ? <button className="company-name-link main-company-name" type="button" onClick={() => onEditCompany(mainCompany.id)}>{mainCompany.name}</button> : "나의 기업"}</h1><p className="main-lead">우리 기업의 수집 현황, 위험 신호와 대응 상태를 한 화면에서 확인합니다.</p></div>
