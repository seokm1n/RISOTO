...
export default function MainPage({ canManageCompanies = true, competitorCompanyLabel = "경쟁사", onOpenCompany, onManageCompanies }) {
  ...
  return <section className="workspace home-workspace">
    ...
    {loading ? <p className="empty-state">기업 정보를 불러오는 중입니다.</p> : <>
      ...
      {mainCompany ? <div className="company-list main-company-list"><CompanyCard company={mainCompany} monitoringSummary={monitoringSummaries[mainCompany.id]} onOpen={onOpenCompany} onEdit={canManageCompanies ? (companyId) => onManageCompanies(companyId, "edit") : undefined} competitorCompanyLabel={competitorCompanyLabel} /></div> : <p className="empty-state">메인 기업 정보가 없습니다.</p>}
      <div className="section-title home-section-title competitor-section-title"><div><span className="eyebrow">COMPETITORS</span><h2>{competitorCompanyLabel}</h2></div><strong>{competitorCompanies.length.toString().padStart(2, "0")}</strong></div>
      {competitorCompanies.length ? <div className="company-list">{competitorCompanies.map((company) => <CompanyCard company={company} key={company.id} monitoringSummary={monitoringSummaries[company.id]} onOpen={onOpenCompany} onEdit={canManageCompanies ? (companyId) => onManageCompanies(companyId, "edit") : undefined} competitorCompanyLabel={competitorCompanyLabel} />)}</div> : <div className="empty-state home-empty"><p>아직 등록한 {competitorCompanyLabel}가 없습니다.</p>{canManageCompanies && <button type="button" onClick={() => onManageCompanies(null, "register")}>{competitorCompanyLabel} 등록하기</button>}</div>}
    </>}
  </section>;
}
