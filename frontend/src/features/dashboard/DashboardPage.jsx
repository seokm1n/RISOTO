...
export default function DashboardPage() {
  ...
  return <section className="workspace"><div className="workspace-head"><div><span className="eyebrow">ANALYTICS / 04</span><h1>통계 대시보드</h1><p>{riskDecisionReady ? `수집량, 감성 흐름과 ${riskModelLabel(riskDetectionStatus)} 위험 판정을 확인합니다.` : "수집량과 감성 흐름을 확인합니다. 최종 위험 판정 모델의 연결을 기다리고 있습니다."}</p></div><label className="range-select">기간<select value={days} onChange={(event) => setDays(Number(event.target.value))}><option value={7}>최근 7일</option><option value={14}>최근 14일</option><option value={30}>최근 30일</option></select></label></div>
