...
export default function CollectionPage({ onOpenCompany }) {
  ...
  const changeAll = async (action) => {
    if (!window.confirm(`모든 기업의 실시간 수집을 ${action === "pause" ? "정지" : "재개"}할까요?`)) return;
    setBusy(`all-${action}`);
  ...
  return <section className="workspace collection-workspace">
    ...
    <section className="panel collection-company-section"><PanelTitle kicker="COMPANY STREAMS" title="기업별 수집 현황" />{loading ? <p className="empty-state">수집 현황을 불러오는 중입니다.</p> : <div className="collection-stream-groups">{companyGroups.map((group) => <section className={`collection-stream-group ${group.role}`} key={group.role}><header><h3>{group.title}</h3></header>{group.companies.length ? <div className="collection-company-list">{group.companies.map(renderCompanyRow)}</div> : <p className="collection-group-empty">등록된 {group.title}이 없습니다.</p>}</section>)}</div>}</section>
  </section>;
}
