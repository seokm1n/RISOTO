...
// 전체 수집기 상태와 기업별 실시간 수집 현황을 조회하고, 관리자에게만 제어 기능을 제공한다.
export default function CollectionPage({ isAdmin, onOpenCompany }) {
  ...
  const changeAll = async (action) => {
    if (!isAdmin || !window.confirm(`모든 기업의 실시간 수집을 ${action === "pause" ? "중지" : "재개"}할까요?`)) return;
    setBusy(`all-${action}`);
  ...
  return <section className="workspace collection-workspace">
    ...
    <div className="collection-summary-grid">
      ...
      <section className="panel collection-control-card"><PanelTitle kicker="COLLECTION CONTROL" title="전체 수집 현황" /><div className="collection-big-stat"><strong>{formatNumber(activeCount)}</strong><span>/ {formatNumber(companies.length)}개 기업 수집 활성</span></div><p>기업별 기사와 댓글을 15분 단위로 수집하고 정제 파이프라인으로 전달합니다.</p>{isAdmin ? <div className="bulk-monitor-controls collection-bulk-controls"><button className="monitor-control stop" type="button" onClick={() => changeAll("pause")} disabled={Boolean(busy)}>{busy === "all-pause" ? "중지 중..." : "전체 중지"}</button><button className="monitor-control start" type="button" onClick={() => changeAll("resume")} disabled={Boolean(busy)}>{busy === "all-resume" ? "재개 중..." : "전체 재개"}</button></div> : <span className="permission-note">수집 제어는 관리자만 사용할 수 있습니다.</span>}</section>
    </div>
    <section className="collection-company-section"><div className="section-title home-section-title"><div><span className="eyebrow">COMPANY STREAMS</span><h2>기업별 수집 현황</h2></div><strong>{companies.length.toString().padStart(2, "0")}</strong></div>{loading ? <p className="empty-state">수집 현황을 불러오는 중입니다.</p> : <div className="collection-company-list">{companies.map((company) => { const summary = summaries[company.id]; const canToggle = ["active", "paused"].includes(company.monitoring_status); return <article className="collection-company-row" key={company.id}><div><span className={`status-dot ${company.monitoring_status}`} /><div><strong>{company.name}</strong><small>{company.industry_name} · {MONITORING_LABELS[company.monitoring_status] ?? company.monitoring_status}</small></div></div><dl><div><dt>정제 기사</dt><dd>{formatNumber(summary?.article_count)}</dd></div><div><dt>분석 완료</dt><dd>{formatNumber(summary?.analyzed_count)}</dd></div><div><dt>마지막 수집</dt><dd>{formatDate(summary?.last_collected_at)}</dd></div></dl><div className="collection-row-actions"><button type="button" onClick={() => onOpenCompany(company.id)}>상세 보기</button>{isAdmin && canToggle && <button className={`collection-toggle ${company.monitoring_status === "paused" ? "start" : "stop"}`} type="button" onClick={() => changeCompany(company)} disabled={Boolean(busy)}>{busy === company.id ? "처리 중..." : company.monitoring_status === "paused" ? "수집 재개" : "수집 중지"}</button>}</div></article>; })}</div>}</section>
    <section className="panel collection-incidents-panel"><PanelTitle kicker="COLLECTION INCIDENTS" title="최근 수집 장애" /><IncidentList incidents={incidents} companies={companies} onAcknowledge={isAdmin ? acknowledgeIncident : undefined} /></section>
  </section>;
}
