...
export function CollectedArticlesDialog({ company, days = null, windowRange = null, onClose }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [company.id, days, displayMode, page, windowRange]);
  ...
  return <div className="collection-articles-modal-layer">
    ...
    <section className="collection-articles-modal" role="dialog" aria-modal="true" aria-labelledby="collection-articles-title">
      <header><div><span className="eyebrow">COLLECTED ARTICLES</span><h2 id="collection-articles-title">{company.name} {windowRange ? `${formatDate(windowRange.start)} 구간 ` : days ? `최근 ${days}일 ` : ""}수집 기사</h2><p>{windowRange ? `${formatDate(windowRange.start)}부터 15분 동안의 ${title}` : `${title}를 최신순으로 확인합니다.`}</p></div><button className="collection-articles-close" type="button" onClick={onClose} aria-label="닫기">×</button></header>
