  ...
  return <div className="notification-layer">
    ...
    <aside className="notification-drawer" id="notification-drawer" role="dialog" aria-modal="true" aria-labelledby="notification-drawer-title">
      ...
      <div className="notification-drawer-list">{allowedItems.length ? allowedItems.map((item) => {
        ...
        return <button className={`notification-drawer-item risk ${isRead ? "read" : "unread"}`} type="button" onClick={() => onRiskOpen(item)} key={item.id}><IconBadge name="risk" className="notification-type-mark" /><div><span>{isRead ? "읽음" : "새 위험 알림"}</span><strong>{item.title}</strong><strong className="notification-article-title risk-event-display-title">{item.message}</strong><small>{item.latest_article_at ? `최근 기사 ${formatDate(item.latest_article_at)}` : formatDate(item.created_at)}</small></div><b aria-hidden="true">→</b></button>;
