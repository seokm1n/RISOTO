...
function NotificationDrawer({ open, onClose, notifications, error, readIds = EMPTY_READ_IDS, markingAllRead = false, onMarkAllRead = NOOP, onRiskOpen = NOOP }) {
  ...
  return <div className="notification-layer">
    ...
    <aside className="notification-drawer" id="notification-drawer" role="dialog" aria-modal="true" aria-labelledby="notification-drawer-title">
      <div className="notification-drawer-head"><div><span className="eyebrow">NOTIFICATION CENTER</span><h2 id="notification-drawer-title">위험 알림</h2><p>최근 관련 기사 날짜 기준, 오늘을 포함한 최근 3일의 위험 이슈입니다.</p></div><button className="notification-close" type="button" onClick={onClose} aria-label="알림 닫기">×</button></div>
      ...
      <div className="notification-drawer-list">{allowedItems.length ? allowedItems.map((item) => {
        ...
        return <button className={`notification-drawer-item risk ${isRead ? "read" : "unread"}`} type="button" onClick={() => onRiskOpen(item)} key={item.id}><span className="notification-type-mark" aria-hidden="true" /><div><span>{isRead ? "읽음" : "새 위험 알림"}</span><strong>{item.title}</strong><strong className="notification-article-title risk-event-display-title">{item.message}</strong><small>{item.latest_article_at ? `최근 기사 ${formatDate(item.latest_article_at)}` : formatDate(item.created_at)}</small></div><b aria-hidden="true">→</b></button>;
