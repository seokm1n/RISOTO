...
function AdminHead({ title, description }) {
...
function MemberPasswordReset({ member, onClose, onComplete }) {
  ...
  return <div className="admin-modal-layer">
    ...
    <form className="admin-modal" onSubmit={submit}>
      <div className="admin-modal-head"><div><h2>회원 비밀번호 재설정</h2><p>{member.email}</p></div><button className="company-edit-close" type="button" onClick={onClose} aria-label="닫기">×</button></div>
...
function MembersAdminView() {
  ...
  return <section className="workspace admin-workspace admin-members-workspace">
    <div className="admin-members-shell">
      <AdminHead title="회원 관리" description="일반 회원의 가입 현황과 등록 기업을 확인하고, 필요한 경우 비밀번호를 재설정합니다." />
      {notice && <div className={`notice ${notice.type}`} role="status">{notice.message}</div>}
      <div className="metric-grid dashboard-metrics admin-metrics"><Metric icon="members" label="일반 회원" value={data.total} /></div>
      <section className="panel admin-members-panel">
        <div className="admin-panel-heading">
          <PanelTitle title="일반 회원 목록" />
...
function CollectionAdminView() {
  ...
  return <section className="workspace admin-workspace">
    <AdminHead title="수집 관리" description="전체 회원의 수집 현황과 수집원 상태를 한 곳에서 확인합니다." />
    {notice && <div className={`notice ${notice.type}`} role="status">{notice.message}</div>}
    {loading && !data ? <p className="empty-state">전체 수집 현황을 불러오는 중입니다.</p> : <>
      <div className="metric-grid dashboard-metrics admin-metrics"><Metric icon="collection" label="수집중" value={data?.active_companies ?? 0} /><Metric icon="companies" label="전체 등록 기업수" value={data?.total_companies ?? 0} /><Metric icon="articles" label="전체 수집량" value={data?.collected_count ?? 0} /><Metric icon="risk" label="전체 위험량" value={data?.risk_count ?? 0} tone={data?.risk_count ? "danger" : "success"} /></div>
      <div className="admin-collection-grid"><section className="panel"><PanelTitle title="전체 수집량·위험량" /><div className="admin-daily-chart" role="img" aria-label="최근 14일 전체 수집량과 위험량"><div className="admin-daily-bars">{(data?.daily ?? []).map((item) => <div className="admin-daily-bar" key={item.day} title={`${item.day} · 수집 ${item.story_count}건 · 위험 ${item.risk_count}건`}><i style={{ height: `${Math.max(item.story_count ? 8 : 3, item.story_count / maxDaily * 100)}%` }} /><b style={{ height: `${Math.max(item.risk_count ? 8 : 3, item.risk_count / maxDaily * 100)}%` }} /><span>{new Date(item.day).toLocaleDateString("ko-KR", { month: "numeric", day: "numeric" })}</span></div>)}</div><div className="admin-chart-legend"><span><i className="collection" />수집량</span><span><i className="risk" />위험량</span></div></div></section><section className="panel"><PanelTitle title="수집 API 관리" /><div className="admin-provider-list">{(data?.providers ?? []).map((provider) => <div key={provider.source}><span>{provider.source}</span><strong className={provider.status === "연결됨" ? "connected" : "disconnected"}>{provider.status}</strong></div>)}</div></section></div>
      <section className="panel admin-company-overview"><PanelTitle title="기업별 수집 현황" /><div className="admin-company-table-wrap"><table className="admin-company-table"><thead><tr><th>기업</th><th>소유 회원</th><th>구분</th><th>상태</th><th>수집량</th><th>위험량</th><th>마지막 수집</th></tr></thead><tbody>{(data?.companies ?? []).map((company) => <tr key={company.id}><td><strong>{company.name}</strong></td><td>{company.owner_email}</td><td>{company.company_role === "main" ? "나의 기업" : "비교 기업"}</td><td><span className={`admin-status-pill ${company.monitoring_status}`}>{MONITORING_LABELS[company.monitoring_status] ?? company.monitoring_status}</span></td><td>{formatNumber(company.article_count)}건</td><td>{formatNumber(company.risk_count)}건</td><td>{formatDate(company.last_collected_at)}</td></tr>)}</tbody></table></div></section>
      <section className="panel admin-incidents-panel"><PanelTitle title="최근 수집 장애" /><AdminIncidentList incidents={data?.incidents} companies={data?.companies} /></section>
    </>}
  </section>;
}
