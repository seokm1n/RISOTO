...
function RiskDetail({ risk, canReview = false }) {
  ...
  return <div className="risk-detail">
    ...
    {content && <div className="response-draft"><div><span className="eyebrow">RESPONSE DRAFT · MANAGEMENT APPROVAL REQUIRED</span><strong>{content.risk_summary}</strong></div>{Object.entries(content.recommended_actions ?? {}).map(([horizon, actions]) => <section key={horizon}><h4>{horizon === "immediate" ? "즉시" : horizon === "within_24h" ? "24시간 이내" : "7일 이내"}</h4>{actions.map((action, index) => <div key={`${horizon}-${index}`}><p>{action.action}</p>{action.evidence_urls?.map((url) => <a href={url} target="_blank" rel="noreferrer" key={url}>근거 {index + 1}</a>)}</div>)}</section>)}<p className="uncertainty">불확실성: {content.uncertainty}</p>{canReview ? <div className="draft-review"><input value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="관리 메모 (선택)" /><button type="button" onClick={() => review("approve")} disabled={loading || latest.approval_state !== "draft"}>승인</button><button type="button" onClick={() => review("reject")} disabled={loading || latest.approval_state !== "draft"}>반려</button><span>{latest.approval_state === "draft" ? "외부 전송·실행 금지" : latest.approval_state === "approved" ? "관리 승인 완료" : "반려됨"}</span></div> : <div className="draft-review readonly"><span>{latest.approval_state === "draft" ? "관리자 승인 대기" : latest.approval_state === "approved" ? "관리 승인 완료" : "반려됨"}</span></div>}</div>}
  </div>;
}
