...
function ManagementNotifications({ data, error, panelRef, onRiskOpen }) {
  ...
  return <section className="panel management-notifications" id="management-notifications" ref={panelRef} tabIndex={-1} aria-labelledby="management-notifications-title">
    <div className="management-notifications-head"><div><h2 id="management-notifications-title">위험 알림</h2><p>확인이 필요한 최종 위험 사건을 확인합니다.</p></div><div className="notification-summary"><span>위험 <strong>{formatNumber(riskCount)}</strong></span></div></div>
...
function AnalysisManagementPage({ notifications, notificationError, focusRequest, onRiskNotificationOpen }) {
  ...
  return <section className="workspace review-workspace"><div className="workspace-head"><div><h1>운영 관리</h1><p>기본 KLUE 모델로 기사를 분석하고, 최종 위험 사건과 수집·분석 품질을 관리합니다.</p></div></div>
