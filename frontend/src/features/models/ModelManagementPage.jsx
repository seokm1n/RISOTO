...
// 운영 모델 버전, 학습 준비도와 일일 품질 점검을 관리자에게 제공한다.
export default function ModelManagementPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [versionsResponse, readinessResponse, checkResponse, statusResponse] = await Promise.all([api.get("/model-versions"), api.get("/model-training-readiness"), api.get("/model-monitoring"), api.get("/risk-detection-status")]);
      setVersions(versionsResponse.data); setReadiness(readinessResponse.data); setModelCheck(checkResponse.data); setRiskStatus(statusResponse.data); setNotice(null);
  ...
  const promote = async (model) => {
    ...
    setBusy(`promote-${model.id}`); setNotice(null);
    try { await api.post(`/model-versions/${model.id}/promote`); await load(); setNotice({ type: "success", message: `${model.version} 모델을 운영 버전으로 승격했습니다.` }); }
    catch (requestError) { setNotice({ type: "error", message: getErrorMessage(requestError) }); }
    finally { setBusy(null); }
  };
  const rerunCheck = async () => {
    setBusy("check"); setNotice(null);
    try { const response = await api.post("/model-monitoring/check"); setModelCheck(response.data); setNotice({ type: "success", message: "수집·분석 품질 점검을 다시 실행했습니다." }); }
    catch (requestError) { setNotice({ type: "error", message: getErrorMessage(requestError) }); }
    finally { setBusy(null); }
  };
  ...
  return <section className="workspace model-workspace">
    <div className="workspace-head"><div><span className="eyebrow">MODEL OPERATIONS / ADMIN</span><h1>모델 관리</h1><p>운영 모델, 후보 승격 조건과 데이터 품질을 관리합니다.</p></div><span className="admin-only-badge">ADMIN ONLY</span></div>
