...
const PLANNED_SOURCES = ["youtube_comment", "dart"];
...
function RealtimePage() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const sourceQuery = sourceFilter ? `&source=${encodeURIComponent(sourceFilter)}` : "";
      const [monitoring, jobs, articles, risks] = await Promise.all([api.get(`/companies/${id}/monitoring`), api.get(`/companies/${id}/collection-jobs?page=${jobPage}&page_size=10`), api.get(`/companies/${id}/articles?page=${articlePage}&page_size=10${sourceQuery}`), api.get(`/companies/${id}/risk-events?limit=10`)]);
      setData({ monitoring: monitoring.data, jobs: jobs.data, articles: articles.data, risks: risks.data }); setError(null);
    } catch (requestError) { setError(getErrorMessage(requestError)); }
  }, [selectedId, articlePage, jobPage, sourceFilter]);
  ...
  return <section className="workspace"><div className="workspace-head"><div><span className="eyebrow">LIVE COLLECTION / 02</span><h1>실시간 수집</h1><p>기사는 15분마다 수집하고, 화면 현황은 15초마다 갱신합니다.</p></div><span className="live-indicator"><i /> LIVE</span></div>
    <div className="monitor-toolbar"><label>모니터링 기업<select value={selectedId} onChange={(event) => { setSelectedId(event.target.value); setArticlePage(1); setJobPage(1); setSourceFilter(""); }}>{companies.map((company) => <option key={company.id} value={company.id}>{company.name} · {company.industry_name}</option>)}</select></label>{showCollectionCountdown && <div className="collection-countdown"><span>다음 기사 수집까지</span><strong>{formatCountdown(secondsUntilCollection)}</strong><small>15분 주기</small></div>}{selected && <><span className={`state-badge ${selected.monitoring_status}`}>{MONITORING_LABELS[selected.monitoring_status]}</span><button className={`monitor-control ${monitorControlClass}`} onClick={changeMonitoringState} disabled={changingState || monitoringPreparing || !monitoringActionAvailable}>{monitoringPreparing && <i className="monitor-loader" aria-hidden="true" />}{monitorControlLabel}</button></>}</div>
    ...
    {!selected ? <p className="empty-state">먼저 기업 등록 페이지에서 모니터링할 기업을 등록해 주세요.</p> : data && <>
      ...
      <div className="live-grid"><section className="panel span-two"><PanelTitle kicker="COLLECTED ARTICLES" title="수집된 기사" /><label className="source-filter">수집 출처<select value={sourceFilter} onChange={(event) => { setSourceFilter(event.target.value); setArticlePage(1); }}><option value="">전체 출처</option>{data.articles.sources.map((source) => <option value={source} key={source}>{SOURCE_LABELS[source] ?? source}</option>)}{PLANNED_SOURCES.filter((source) => !data.articles.sources.includes(source)).map((source) => <option value={source} key={source} disabled>{SOURCE_LABELS[source]} · 수집 데이터 없음</option>)}</select></label><div className="article-list">{data.articles.items.length ? data.articles.items.map((article) => <a className="article-row" key={article.id} href={article.url} target="_blank" rel="noreferrer"><span className={`sentiment-pill ${article.sentiment_label ?? "pending"}`}>{sentimentText(article.sentiment_label)}</span><div><strong>{article.title}</strong><small>{SOURCE_LABELS[article.source] ?? article.source} · {formatDate(article.published_at ?? article.created_at)}</small></div>{article.is_anomaly && <span className="risk-mark">위험</span>}</a>) : <p className="panel-empty">선택한 출처에서 수집된 기사가 없습니다.</p>}</div><Pagination page={data.articles.page} pageSize={data.articles.page_size} total={data.articles.total} onChange={setArticlePage} /></section>
