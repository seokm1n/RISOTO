"""Fine-tune a binary classifier that answers a different question than the spam/ad
model: "is this text genuinely, substantively about the tagged company (its business,
products, actions, risk) versus an incidental mention (name collision, sports-team
homonym, generic digest/aggregator, trivial personal mention)?"

Labels came from manually reading ~500 individual real mentions sampled from the
legacy raw collection export, stratified to include known-confusable patterns (e.g.
SSG Landers baseball vs SSG the company).
"""
...
def _load_csv_rows(csv_path: Path) -> list[dict]:
    ...
    return [row for row in rows if row.get("text", "").strip() and row.get("label") in LABELS]
...
def train_topical_relevance_from_csv(
    csv_path: Path,
    output_root: Path,
    ...
    class CsvDataset(Dataset):
        ...
        def __getitem__(self, index):
            ...
            return {
                ...
                "labels": torch.tensor(LABELS[item["label"]]),
            }
