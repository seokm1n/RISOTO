...
# 반사이익은 다루기 까다롭다. "비교 기업 악재로 고객을 유치할 기회"라는 문장이 외부로
# 새면 그 자체가 새 리스크가 된다. 그래서 기회를 '비교 기업 손실 활용'이 아니라
# '선점 가능한 개선·홍보 항목'으로 프레이밍하도록 지시한다.
_DIRECTION_RULE_UPSIDE = (
    "- 이 사안은 {main_company}에 **반사이익** 방향입니다. 다만 기회를 "
    "'비교 기업 사고로 생긴 이탈 고객 유치'로 쓰지 마세요. 이 문서가 외부로 새면 그 문장\n"
    "  자체가 평판 리스크가 됩니다. '우리가 먼저 갖춰 알릴 수 있는 항목'으로 쓰세요.\n"
    "- avoid에 **비교 기업 사고를 직접 언급하는 마케팅·홍보 금지**를 반드시 포함하세요.\n"
...
def verify_recommendation(rec: dict, peer: dict) -> list[str]:
    ...
    # 규칙 4 - 반사이익인데 경고가 없는가.
    #   비교 기업 악재를 활용하는 문장이 외부로 새면 그 자체가 리스크다. 방향이 반사이익일
