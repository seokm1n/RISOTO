...
def embed(texts: list[str], model: str | None = None) -> tuple[np.ndarray, dict[str, int]]:
...
def embed_one(text: str, model: str | None = None) -> np.ndarray:
    vectors, _ = embed([text], model=model)
    return vectors[0]
