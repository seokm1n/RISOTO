...
- summary_points는 **요점 3~5개**입니다. 하나에 한 가지 내용만 담고, 각 1~2문장으로
  끊어 쓰세요. 긴 줄글 한 덩어리로 쓰지 마세요. 순서는 (1) 무슨 일이 벌어지고 있는지,
  (2) 얼마나 심각하고 빠른지, (3) 핵심 쟁점이 무엇인지, (4) 그래서 무엇을 먼저 해야 하는지
  흐름을 권장합니다.
- judgment_basis는 왜 이 사안을 리스크로 판단했는지 서술합니다. 수치 근거와 실제 반응
  내용을 함께 엮되, 위 [읽는 사람] 규칙을 지키세요.
...
- limitations(한계 고지)에는 표본 한계와 미확인 사항을 적으세요. 이 보고서는 법률 자문이
  아니라는 점을 반드시 포함하세요.
{case_rule}
{regulation_rule}
{stance_block}"""
...
_CASE_RULE_ALLOWED = (
    "- 과거 유사 사례는 **case_insights 필드에만** 쓰세요. judgment_basis나 전략 detail\n"
    "  본문에 사례 이야기를 섞지 마세요. 사례가 여기저기 흩어지면 읽는 사람이 지금 우리\n"
    "  이야기인지 남의 사례인지 구분하기 어려워집니다.\n"
...
_RAG_HEADER = (
    "\n[상황별 보충 지침 - 참고용]\n"
    "아래는 위 대응 원칙의 근거 자료에서 이번 상황과 가까운 대목을 찾아온 것입니다. "
    "원칙과 충돌하면 원칙이 우선입니다. 각 항목에 붙은 한계 표시를 지켜 인용하세요.\n"
)
...
def build_user_prompt(payload: AlertPayload, ev: Evidence) -> str:
    ...
    if ev.mentions:
        ...
        for m in ev.mentions:
            ...
            lines.append(f"- [{m.mention_id}] ({' / '.join(meta)}) {m.text[:300]}")
