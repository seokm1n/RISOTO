...
- 각 시나리오는 지정된 [관점]에서 무엇을 우선할지 독립적으로 판단하세요.
- 관점의 차이를 만들기 위해 [근거]에 없는 사실이나 조치를 추가하지 마세요.
- 서로 다른 관점에서 검토하더라도 [근거]상 같은 대응이 적절하다면, 억지로 다른 결론을 만들지 마세요.
- scenario_headline은 이 대응안의 핵심 방향을 한눈에 보여주는 **짧은 이름**입니다.
  12자 안팎의 명사형으로 작성하고, "~하기", "~한다" 같은 서술형은 쓰지 마세요.
  이번 사안에서 제안하는 핵심 대응 방향이나 조치를 구체적으로 드러내되, 담당자가
  한눈에 알아볼 말로 쓰세요.
  [근거]에 없는 조치를 새로 만들어 넣지 마세요. 내부 용어나 영어는 쓰지 마세요.
  예: "제품 회수와 고객 안내", "사실관계 확인 우선", "피해자 보호 우선"
- scenario_recommendation은 **이 안을 택했을 때 담당자가 지금 할 일**을 1~2문장으로
  적습니다. 화면 상단에 "현재 권고"로 걸리는 문장이라, 이것만 읽고도 무엇부터 하면
  되는지 알 수 있어야 합니다. 명사 나열이 아니라 문장으로 쓰고 "~하세요"로 맺으세요.
  예: "결함·표시·리콜 관리 현황과 법정 보고 요건을 먼저 확인한 뒤, 확인 결과에 근거해
  대외 응답과 규제 대응을 준비하세요."
...
SCENARIO_STANCES: dict[str, str] = {
    "선제_대응": (
        "정보가 모두 확정되기를 기다리기보다 현재 확인된 사실을 바탕으로 "
        "실행 가능한 대응을 먼저 시작하는 관점입니다. "
        "구체적인 조치는 반드시 [근거]에서 확인할 수 있는 범위 안에서 제안하고, "
        "확인된 사실과 아직 확인되지 않은 내용을 구분합니다. "
        "감수하는 것: 추가 확인에 따라 초기 안내나 조치가 수정될 수 있습니다."
    ),

    "검증_우선": (
        "즉각적인 대응 확대보다 사실관계와 대응 근거를 충분히 확인한 뒤 "
        "대응 범위를 결정하는 관점입니다. "
        "[근거]에 신고·통지 등 반드시 지켜야 할 기한이 제시되어 있다면 이를 준수합니다. "
        "감수하는 것: 확인에 시간이 걸리면 대응 지연이나 정보 공백에 대한 비판이 발생할 수 있습니다."
    ),

    "이해관계자_보호": (
        "대응 속도나 원인 규명보다 이번 사안으로 직접 영향을 받는 이해관계자의 "
        "안전, 피해, 불편 또는 권익을 우선하여 대응 범위를 결정하는 관점입니다. "
        "구체적인 보호·지원 조치는 반드시 [근거]에서 확인할 수 있는 범위 안에서 제안합니다. "
        "감수하는 것: 보호 범위를 넓게 잡을수록 비용이나 운영 부담이 커질 수 있습니다."
    ),
}

DEFAULT_STANCES = ("선제_대응", "검증_우선", "이해관계자_보호")
