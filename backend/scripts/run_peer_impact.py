...
def _print_prompts(payload: AlertPayload) -> None:
    ...
    def spy(*, system, user, schema, schema_name):
...
def _peer_dict(raw: dict, payload: AlertPayload, analysis: dict) -> dict:
...
def print_table(rows: list[tuple[dict, dict]]) -> None:
    ...
    print("-" * 96)
    for raw, peer in sorted(rows, key=lambda rp: -(
            ((rp[0].get("_provenance") or {}).get("peer_mention_ratio") or {}).get("ratio") or -1)):
...
def main() -> None:
    ...
    if not args.dry_run and not os.environ.get("DATABASE_URL"):
        # run_peer_recommend.py와 같은 안내. 접속은 하지 않고 설정 로딩에만 쓰인다.
        sys.exit(
    ...
    for path in args.payloads:
        ...
        peer = _peer_dict(raw, payload, analysis)
        ...
        if args.out_dir:
            ...
            stem = Path(path).stem.replace("alert_peer_", "impact_peer_")
