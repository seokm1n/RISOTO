...
> **업데이트 (같은 날, master 병합 후)**: 이 문서를 커밋하려던 시점에 팀원이 정확히 같은 문제를
> 독립적으로 파고들어 `backend/training_data/HUMAN_LABELS.md`를 올렸습니다. 아래 1번 항목에서
> "확정 라벨 부족으로 검증 불가"라고 진단한 것과 **정확히 같은 원인**("시간순 분할이 양성 사건을
> 전부 앞쪽에 몰아넣어 검증 구간이 100% 음성이었다")을 독립적으로 확인했고, 사람이 직접 매긴
> 정답지(관련성 600건, 위기 사건 82건)로 제대로 된 평가까지 마쳤습니다. 그 결과가 아래 1번
> 항목 끝에 추가돼 있습니다 — **관련성 필터가 생각보다 약하다는 새로운 사실**이 나왔습니다.
...
### 1-1. (병합 후 업데이트) 사람 정답지로 제대로 측정한 실제 성능

`backend/training_data/HUMAN_LABELS.md`(팀원이 같은 날 추가)에서 event-group 층화 K-fold
out-of-fold 방식으로 제대로 측정한 결과입니다 — 위 문제(시간순 분할이라 검증 구간에 positive가
없던 것)를 정확히 우회한 방법입니다.

- **위기 탐지(LightGBM)는 실제로 꽤 괜찮습니다**: Brier 0.0229, AUC 0.9956, 재현율 0.837.
  다만 모델이 자체 저장한 지표(Brier 0.00786)는 **2.9배 낙관적으로 부풀려져 있었습니다** —
  운영 중인 모델의 `model_versions.metrics`를 그대로 믿으면 안 됩니다.
- **⚠️ 관련성 필터가 생각보다 약합니다**: 정확도 58.0%, **정밀도 48.7%**, 재현율 72.8%,
  AUC 0.7182. 통과시킨 기사의 절반 이상이 사실 무관한 기사라는 뜻입니다. 원인도 구체적으로
  나왔습니다 — **제목에 기업명이 있으면 유효율 71%, 본문에만 있으면 32%**인데, 통과분의 60%가
  "본문에만 기업명이 있는" 경우입니다. `article_filtering.py`의 관련성 판정에서 제목 언급 여부에
  더 높은 가중치를 주는 방향이 다음 개선 후보로 보입니다.
- 이 정답지는 아직 `article_labels`/`risk_event_labels` DB 테이블에 반입되지 않은 **CSV
  상태**입니다. 다음 사람이 할 일: annotator를 실제 검수자 이름으로 넣는 반입 스크립트를 작성해서
  DB에 넣으면, 이후 재학습·평가가 자동으로 이 정답지를 쓰게 됩니다.
...
- 광고 탐지 최초 실측: `reason=="advertisement"`로 직접 잡은 것만 보면 정밀도 75.8% / 재현율
  22.7%로 낮아 보였는데, **처음 이 숫자만 보고 "광고 10건 중 8건이 통과한다"고 잘못 적었습니다.
  정정합니다** — AD_PATTERNS가 놓쳐도 대부분 커뮤니티/핫딜 게시판 호스트 차단이나 관련성 미달로
  다른 규칙이 대신 걸러내고 있었고, 실제로 accepted까지 도달한 광고는 110건 중 **5건(4.5%)**뿐입니다.
  이 5건은 전부 제목에 회사/제품명이 있어 relevance 하한선(0.70~0.76)에 걸린 자사몰 상품목록·채용
  공고·블로그 리뷰였습니다 — AD_PATTERNS 자체보다 이 하한선 쪽을 다음에 볼 만합니다.
...
6. **relevance 하한선(identity/product_in_title일 때 0.70~0.76 floor)에 "비-뉴스 페이지" 신호를
   추가해주세요** — 실제로 새어나간 광고 5건은 전부 이 하한선 때문에 자사몰·채용공고·블로그 리뷰가
   그대로 통과한 경우였습니다. AD_PATTERNS 자체는 재현율이 낮아도 다른 규칙이 대부분 대신 걸러주고
   있어 우선순위는 낮습니다.
